...
class ImageRecorder(object):
    ...
    def __init__(self, sphereW, sphereH, view_angle=65.5, imgW=480/2):
    ...
    def _warp_image(self, Px, Py, frame):
        ...
        return warped_image, Px, Py
        '''
        full_image = interp2d((y_grid, x_grid), im[:,:,:], bounds_error=False, method='linear', fill_value=None)
        warped_image = full_image(samples).reshape(Px.shape[0], Px.shape[1], im.shape[2])
        return warped_image, Px, Py

class VideoRecorder(ImageRecorder):
    def __init__(self, video_in, video_out, x0=0., y0=0.):
        ...
        super(VideoRecorder, self).__init__(sphereW, sphereH)
        self.video = video
        self._current = 0
...
class VirtualCamera(object):
    def __init__(self, video_in, video_out, distance="5s"):
        video = cv2.VideoCapture(video_in)
    ...
    def forward(self, diff_x=0, diff_y=0):
        finished = False
    ...
    def __capture_next(self):
        ...
        if not ret:
            ...
            warped_image = self.__recorder.catch(self.__x, self.__y, frame)
            warped_image[warped_image>255.] = 255.
            warped_image[warped_image<0.] = 0.
            warped_image = warped_image.astype(np.uint8)
        return warped_image
